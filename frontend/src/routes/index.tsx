  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Ôn tập thông minh — Tìm đúng lỗ hổng kiến thức" },
      {
        ...
        content: "Phát hiện điểm yếu, hiểu vì sao sai, tới đúng nội dung học và kiểm tra lại hiểu biết.",
      },
...
function ReviewApp() {
  ...
  const chatContext: ChatContext = {
    label:
      step === "lesson"
        ? `${LESSONS[topic].lesson} · ${LESSONS[topic].slides[slide]!.title}`
        : step === "check"
  ...
  const goLesson = (t: TopicId) => {
    ...
    setStep("lesson");
    aiSay(
      `Mình đã mở ${LESSONS[t].lesson} — ${LESSONS[t].slides[0]!.title}. Đây đúng là phần liên quan tới lỗi sai của bạn. Bạn cứ đọc, có gì hỏi mình ngay tại đây nhé.`,
    );
  };
      ...
      <main className="mx-auto grid max-w-7xl gap-6 px-6 py-6 lg:grid-cols-[1fr_380px]">
        <section className="min-w-0 space-y-4">
                ...
                {WEEKS.map((w) => (
                  ...
                  >
                    ...
                    <div className="mt-3 flex flex-wrap gap-1.5">
                      ...
                      {!w.available && (
                        <span className="text-xs text-muted-foreground">· Bản demo: chọn Tuần 2</span>
                      )}
                    </div>
                  </button>
                ))}
          ...
          {step === "test" && (
            <>
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold text-foreground">Bài kiểm tra nhanh — {week.title}</h2>
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  disabled={Object.keys(answers).length < weekQuestions.length}
            ...
            <>
              <div className="rounded-xl border border-border bg-card p-5">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <h2 className="text-xl font-semibold text-foreground">Kết quả & lỗ hổng kiến thức</h2>
                    <p className="mt-1 text-sm text-muted-foreground">
                      Đúng {correctCount}/{weekQuestions.length} câu. Quan trọng hơn điểm số: bạn nên ôn gì trước.
                    </p>
                  </div>
                    ...
                    >
                      <div>
                        <p className="text-sm font-medium text-foreground">
                          {TOPICS[t].name}
                          {wrongByTopic[t] ? ` — sai ${wrongByTopic[t]} câu` : " — không sai câu nào"}
                        </p>
            ...
            <>
              ...
              <div className="rounded-xl border border-border bg-card p-5">
                ...
                <h2 className="mt-1 text-xl font-semibold text-foreground">
                  {LESSONS[topic].slides[slide]!.title}
                </h2>
                {LESSONS[topic].slides[slide]!.note && (
                  <p className="mt-2 rounded-lg bg-accent/40 px-3 py-2 text-sm text-accent-foreground">
                    {LESSONS[topic].slides[slide]!.note}
                  </p>
                )}
                <div className="mt-4 space-y-3">
                  {LESSONS[topic].slides[slide]!.body.map((p, i) => (
                ...
                <div className="mt-5 flex flex-wrap items-center gap-2">
                  ...
                  <button
                    disabled={slide >= LESSONS[topic].slides.length - 1}
                    onClick={() => setSlide((s) => s + 1)}
                    className="rounded-lg border border-border px-3 py-1.5 text-sm disabled:opacity-40"
                  >
                    Xem phần tiếp theo
                  ...
                  <span className="text-xs text-muted-foreground">
                    Slide {slide + 1}/{LESSONS[topic].slides.length}
                  </span>
                  <button
                    onClick={startCheck}
          ...
          {step === "summary" && (
            <>
              <div className="rounded-xl border border-border bg-card p-5">
                <h2 className="text-xl font-semibold text-foreground">Tổng kết tuần — {week.title}</h2>
                <p className="mt-1 text-sm text-muted-foreground">
                ...
                <div className="mt-4 rounded-lg bg-secondary p-4 text-sm text-foreground">
                  <p>
                    <strong>Đã ôn:</strong>{" "}
                    {week.topics.filter((t) => topicStatus(t) === "reviewed").map((t) => TOPICS[t].name).join(", ") ||
                      "chưa có"}
                  </p>
                  <p className="mt-1">
...
function CheckPanel(props: {
  ...
  return (
    <>
      <button onClick={props.onBackToLesson} className="text-sm text-muted-foreground hover:text-primary">
          ...
          {q.options.map((o, i) => {
            const state = props.submitted
              ...
              : props.answer === i
                ...
                : "border-border";
            return (
              <label key={i} className={`flex cursor-pointer items-center gap-2 rounded-lg border p-2.5 text-sm ${state}`}>
                <input
                  type="radio"
                  name="check"
          ...
          <div className="mt-4 space-y-3">
            ...
            >
              <p className="font-semibold">
                {ok ? "Chính xác — khái niệm này đã được xác nhận hiểu." : "Chưa đúng — mình chưa đánh dấu là đã hiểu."}
              </p>
            ...
            <div className="flex flex-wrap gap-2">
              {!ok && (
                <>
                  <button onClick={props.onBackToLesson} className="rounded-lg border border-border px-3 py-1.5 text-sm hover:border-primary">
                    Xem lại phần này
                  </button>
                  <button onClick={props.onExplainDifferently} className="rounded-lg border border-border px-3 py-1.5 text-sm hover:border-primary">
                    Giải thích theo cách khác
                  </button>
                  <button onClick={props.onAnotherExample} className="rounded-lg border border-border px-3 py-1.5 text-sm hover:border-primary">
                    Cho ví dụ khác
                  </button>
                  <button onClick={props.onRetryOther} className="rounded-lg border border-border px-3 py-1.5 text-sm hover:border-primary">
                    Thử lại với câu khác
                  </button>
                  <button onClick={props.onLater} className="rounded-lg border border-border px-3 py-1.5 text-sm hover:border-primary">
                    Để ôn sau
                  </button>
                </>
              )}
              {ok && (
                <>
                  ...
                  <button onClick={props.onNext} className="rounded-lg border border-border px-3 py-1.5 text-sm hover:border-primary">
                    Xem tổng kết tuần
                  </button>
                </>
              )}
